...
class MethodTable
{
	...
	public static function create($className, $servicePath = NULL, &$classComment){
		...
		if(file_exists($className))
		{
			...
			$className = substr($className, 1); // Retira o prefixo t ex.: tCliente.php => Cliente
		}
		else
		{
			$className = str_replace('.php', '', $className);
	...
	public static function getClassMethodsTokenizer($sourcePath, $className, & $classComment)
	{
