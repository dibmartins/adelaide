...
// the xajax and AjaxResponse classes
...
class Ajax
{
	 ...
	 */
	function Ajax($sRequestURI="",$sWrapperPrefix="xajax_",$sEncoding=XAJAX_DEFAULT_CHAR_ENCODING,$bDebug=false)
	{
	...
	{	
		...
		if ($this->sPreFunction) {
			if (!$this->IsFunctionCallable($this->sPreFunction)) {
		...
		if ($bFoundFunction)
		{
			for ($i = 0; $i < sizeof($aArgs); $i++)
			{
				...
				{
					$aArgs[$i] = $this->DecodeUTF8Data($aArgs[$i]);	
				}
			}

			if ($this->sPreFunction) {
				$mPreResponse = $this->CallFunction($this->sPreFunction, array($sFunctionNameForSpecial, $aArgs));
			...
			if (!$bEndRequest) {
				if (!$this->IsFunctionCallable($sFunctionName)) {
					$objResponse = new AjaxResponse();
					$objResponse->AddAlert("A função registrada $sFunctionName não pode ser encontrada.");
					$sResponse = $objResponse->GetXML();
				}
				else {
					...
					$sResponse = $this->CallFunction($sFunctionName, $aArgs);
				}
				...
				if (!is_string($sResponse) || strpos($sResponse, "<xjx>") === FALSE) {
					$objResponse = new AjaxResponse();
					$objResponse->AddAlert("Nenhuma resposta XML retornada pela função $sFunctionName.");
					$sResponse = $objResponse->GetXML();
				}
				else if ($sPreResponse != "") {
					$sNewResponse = new AjaxResponse($this->sEncoding, $this->bOutputEntities);
		...
		if ($this->bErrorHandler && !empty( $GLOBALS['XajaxErrorHandlerText'] )) {
			...
			if ($this->sLogFile) {
				...
				if (!$fH) {
					$sErrorResponse->AddAlert("** Log Error **\n\nXajax está impossibilitado de escrever no arquivo de log:\n" . $this->sLogFile);
				}
				else {
	 ...
	 */
	function PrintJavascript($sJsURI="../Web/Js/", $sJsFile=NULL)
	{
	...
	function GetJavascriptConfig($additionalRequestURI=false)
	{
		...
		/*     added by charles.a.peterson @ ndsu.edu
			  prefixing requesturi to allow multiple servers.
				 this has been updated in js file and Wrap function also.
		*/
		...
		foreach($this->aFunctions as $sFunction => $bExists) {
			$html .= $this->Wrap($sFunction,$this->aFunctionRequestTypes[$sFunction]);
		}
	...
	function GetJavascriptInclude($sJsURI="", $sJsFile=NULL)
	{
		...
		$html .= "\t<script type=\"text/javascript\">\n";
		$html .= "window.setTimeout(function () { if (!xajaxLoaded) { alert('Erro: O javascript Xajax não pode ser incluído. A URL está incorreta?\\nURL: {$sJsURI}{$sJsFile}'); } }, 500);\n";
		$html .= "\t</script>\n";
		return $html;
	}
	 ...
	 * @return boolean
	 */
	function IsObjectCallback($sFunction)
	{
	 ...
	 * @return boolean
	 */
	function IsFunctionCallable($sFunction)
	{
		if ($this->IsObjectCallback($sFunction)) {
	 ...
	 */
	function CallFunction($sFunction, $aArgs)
	{
		if ($this->IsObjectCallback($sFunction)) {
	 ...
	 * @return string
	 */
	function Wrap($sFunction,$sRequestType=XAJAX_POST)
	{
	...
	function ParseObjXml($rootTag)
	{
						...
						if(stristr($this->aObjArray[$this->iPos],"<v>"))
						{
							...
							while(!stristr($this->aObjArray[$this->iPos],"</v>"))
							{
								...
								else
								{
									...
									{
										$value = $this->DecodeUTF8Data($value);
									}
								}
								$this->iPos++;
							}
		...
		if ($rootTag == "xjxquery")
		{
			...
			if ($this->bDecodeUTF8Input)
			{
				...
				{
					$aArray[$key] = $this->DecodeUTF8Data($value);
				}
			}
...
function XajaxErrorHandler($errno, $errstr, $errfile, $errline)
{
	...
	$GLOBALS['XajaxErrorHandlerText'] .= "\n----\n[$errTypeStr] $errstr\nErro na linha $errline no arquivo $errfile";
}
